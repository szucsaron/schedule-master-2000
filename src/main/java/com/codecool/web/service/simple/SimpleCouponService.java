...
public final class SimpleCouponService implements CouponService {
    ...
    public List<Coupon> getCoupons(User user) throws SQLException {
        return couponDao.find(user.getId());
    }

    public List<Coupon> getCoupons(User user, Shop shop) throws SQLException {
        return couponDao.find(user.getId(), shop.getId());
    }
    ...
    @Override
    public Coupon addCoupon(User user, String name, String percentage) throws SQLException, ServiceException {
        try {
            return couponDao.add(name, user.getId(), Integer.parseInt(percentage));
        } catch (NumberFormatException ex) {
